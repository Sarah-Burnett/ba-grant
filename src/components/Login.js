...
const LoginPage = styled.div`
    ...
    form {
        ...
        grid-template-rows: 1fr auto auto .5fr 1fr;
        align-items: center;
        justify-content: center;
        position: relative;
        ...
        <LoginPage>
            <form onSubmit={loginSubmit}>
                ...
                <div className="password">
                    { !showPassword && <input type="password" value={loginInput} onChange={loginChange}/> }
                    { showPassword && <input value={loginInput} onChange={loginChange}/> }
                    <FontAwesomeIcon className="passIcon" icon={passIcon()} onClick={togglePassword} fixedWidth/>
                </div>
                <p className="error">{loginError && <span><FontAwesomeIcon icon={Warning}/> Password incorrect</span>}</p>
                <Button>Login</Button>
            </form>
        </LoginPage>
    )
